...
def time_build_system_executor(make_env):
    ...
    with subprocess.Popen(['/usr/bin/time', '--version'], stdout=subprocess.PIPE) as p:
        stdout, _ = p.communicate(timeout=1)

        stdout_decoded = stdout.decode('utf-8').rstrip() if stdout else None
        if p.returncode == 0 and stdout_decoded:
            result['USR-BIN-TIME_version'] = stdout_decoded
...
time_kwargs = {'build_system_func': time_build_system_executor, 'exec_func': time_executor}
