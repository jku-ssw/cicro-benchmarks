    ...
    unsigned int iterationsCount)
{
    ...
    chayai_console_change_color(OUTPUT_STREAM, CHAYAI_TEXT_DEFAULT);
    fputs("(", OUTPUT_STREAM);
    if(warmupRunsCount > 0) {
        fprintf(OUTPUT_STREAM, "%u ", warmupRunsCount);
        fputs((warmupRunsCount == 1) ? "warmup, " : "warmups, ", OUTPUT_STREAM);
    }
    ...
    const CHayaiBenchmarkResult* result)
{
    ...
    const double standardDerivation = sqrt(variance);
    const double standardDerivationPercentage = standardDerivation * 100.0 / runTimeAvgUs; // 68% of all values have by maximum this derivation from the average
    ...
    chayai_console_change_color(OUTPUT_STREAM, CHAYAI_TEXT_DEFAULT);
    fprintf(OUTPUT_STREAM, "       Average time: %"FP_TIME_STR"f us (sd: %"FP_TIME_STR"f us / %"FP_TIME_STR"f %%)\n", runTimeAvgUs, standardDerivation, standardDerivationPercentage);
